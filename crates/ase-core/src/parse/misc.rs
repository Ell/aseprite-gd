...
pub fn parse_cel_extra(r: &mut Reader) -> Result<Option<CelExtra>> {
    ...
    Ok(Some(CelExtra { x, y, width, height }))
}
...
pub fn parse_color_profile(r: &mut Reader) -> Result<ColorProfile> {
    ...
    Ok(match kind {
        ...
        2 => {
            ...
            ColorProfile::Icc { fixed_gamma, data: r.bytes(len)?.to_vec() }
        }
        _ => return Err(ParseError::Invalid { offset: start, what: "color profile type" }),
    })
}
...
pub fn parse_external_files(r: &mut Reader) -> Result<Vec<ExternalFile>> {
    ...
    if n as usize > r.remaining() / 12 {
        return Err(ParseError::Invalid { offset: r.pos(), what: "external file count" });
    }
