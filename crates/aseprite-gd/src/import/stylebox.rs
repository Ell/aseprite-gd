//! StyleBoxTexture importer: a 9-patch slice becomes a StyleBoxTexture whose
//! margins come from the slice's center rect. Default slice: the first one
//! with a center; override via the slice_name option.
...
pub struct AseStyleBoxImporter {
    base: Base<EditorImportPlugin>,
}

#[godot_api]
impl IEditorImportPlugin for AseStyleBoxImporter {
    fn get_importer_name(&self) -> GString {
        "aseprite_gd.stylebox".into()
    }

    fn get_visible_name(&self) -> GString {
        "StyleBoxTexture (Aseprite)".into()
    }
    ...
    fn get_import_options(&self, _path: GString, _preset_index: i32) -> Array<AnyDictionary> {
        ...
        let mut slice_name = VarDictionary::new();
        slice_name.set(&"name".to_variant(), &"slice_name".to_variant());
        slice_name.set(&"default_value".to_variant(), &"".to_variant());
        opts.push(slice_name.upcast_any_dictionary());
    ...
    fn import(
        ...
        let slice_name = options
            .get(&"slice_name".to_variant())
            .map(|v| v.to_string())
            .unwrap_or_default();
        let frame = options
