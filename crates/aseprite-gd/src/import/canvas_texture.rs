//! CanvasTexture importer for lit pixel art: diffuse from ordinary layers,
//! normal/specular maps from layers named (or suffixed) "normal",
//! "specular"/"emission" — no manual steps.
...
pub struct AseCanvasTextureImporter {
    base: Base<EditorImportPlugin>,
}

#[godot_api]
impl IEditorImportPlugin for AseCanvasTextureImporter {
    fn get_importer_name(&self) -> GString {
        "aseprite_gd.canvas_texture".into()
    }

    fn get_visible_name(&self) -> GString {
        "CanvasTexture (Aseprite)".into()
    }
    ...
    fn import(
        ...
        let library = match convert::build_canvas_texture(&file, frame) {
            Ok(l) => l,
            Err(e) => {
                godot_error!("aseprite-gd: {source_file}: {e}");
