...
- [ ] Named-region extraction: named slices or named tiles → an owned folder of `AtlasTexture` files over one shared sheet (`extract_dir`); a synced TileSet can reference that same sheet (`sync_with_sheet`) so tilemap and extracted sprites share one texture. Tile names are authored with a bundled Aseprite dialog script (Aseprite has no UI for per-tile user data)
- [ ] Grid splitting: `split_grid` for asset-pack sheets — cells become indexable frames on single-frame files, per-cell animation sets on multi-frame files, and `<tag>_<cell>` sets when combined with tags
